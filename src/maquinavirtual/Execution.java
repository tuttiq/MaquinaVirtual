...
public class Execution {
    ...
    public void execute(Instruction instruction, ArrayList<StackElement> stack, JTextArea txtSaida)
    {
        if(instruction.getOperation().equals("LDC"))
        {    //S:=s + 1 ; M [s]: = k
            stack.add(new StackElement(stack.size(), Integer.parseInt(instruction.getAttr1()) ));
        }
        ...
        {
             stack.add(new StackElement(stack.size(), i+1));
             i = Integer.parseInt(instruction.getAttr1());
        ...
        {
             i = stack.get(stack.size()-1).getValue()-1;
             stack.remove(stack.size()-1);
